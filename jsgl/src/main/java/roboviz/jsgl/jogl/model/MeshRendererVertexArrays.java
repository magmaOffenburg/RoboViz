...
public class MeshRendererVertexArrays implements MeshRenderer {
    ...
    public void setState(GL glContext) {
        ...
        if (mesh.useNormals) {
            ...
            gl.glNormalPointer(GL.GL_FLOAT, vertStride,
                    vBuffer.position(normalOffset / 4));
        }

        if (mesh.useTexCoords) {
            gl.glEnableClientState(GLPointerFunc.GL_TEXTURE_COORD_ARRAY);
            gl.glTexCoordPointer(2, GL.GL_FLOAT, vertStride,
                    vBuffer.position(texCoordOffset / 4));
        }
    }
