...
public class Texture2D implements GLDisposable {
     ...
     */
    public static Texture2D create(GL gl, int w, int h, int format,
     ...
     */
    public void texImage(GL gl, int level, int internalFormat, int width,
            int height, int border, int format, int type, Buffer data) {
        this.w = width;
        this.h = height;

        gl.glTexImage2D(GL.GL_TEXTURE_2D, level, internalFormat, width, height,
    ...
    public static ByteBuffer readPixels(BufferedImage img, boolean alpha)
    ...
    public static Texture2D loadTexMipmaps(GL gl, GLU glu, BufferedImage img) {
        ...
        gl.glTexParameteri(GL.GL_TEXTURE_2D, GL2ES1.GL_GENERATE_MIPMAP,
                GL.GL_TRUE);
        glu.gluBuild2DMipmaps(GL.GL_TEXTURE_2D, format, w, h, format,
                GL.GL_UNSIGNED_BYTE, pixelData);
        gl.glBindTexture(GL.GL_TEXTURE_2D, 0);
    ...
    public static Texture2D loadTex(GL gl, BufferedImage img) {
        ...
        // upload pixel data to texture object
        int format = alpha ? GL.GL_RGBA : GL.GL_RGB;
        gl.glTexImage2D(GL.GL_TEXTURE_2D, 0, format, w, h, 0, format,
                GL.GL_UNSIGNED_BYTE, pixelData);
        gl.glBindTexture(GL.GL_TEXTURE_2D, 0);
    ...
    public static Texture2D loadTexMipmaps(GL gl, GLU glu, String name,
            ClassLoader loader) {
        BufferedImage img;
        try {
     ...
     * @return
     */
    public static Texture2D create(GL gl, int w, int h, int internalFormat,
            int format, int type, int bytesPerPixel) {
        ...
        gl.glTexImage2D(GL.GL_TEXTURE_2D, 0, internalFormat, w, h, 0, format,
                type, data);
    ...
    public static Texture2D createTexture(GL gl, int w, int h,
            int internalFormat, int bytesPerPixel) {
        // storage space for texture data - 4 bytes per pixel (RGBA)
        ByteBuffer data = Buffers.newDirectByteBuffer(w * h * bytesPerPixel);
        data.limit(data.capacity());
        ...
        gl.glTexImage2D(GL.GL_TEXTURE_2D, 0, internalFormat, w, h, 0,
                internalFormat, GL.GL_UNSIGNED_BYTE, data);
