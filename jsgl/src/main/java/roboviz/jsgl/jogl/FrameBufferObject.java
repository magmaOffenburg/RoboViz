...
public class FrameBufferObject implements GLDisposable {
     ...
     */
    public static FrameBufferObject create(GL gl, int w, int h,
            int internalFormat) {
        ...
        colorTex.texImage(gl, 0, internalFormat, w, h, 0, GL.GL_RGBA,
     ...
     */
    public static FrameBufferObject create(GL gl, int w, int h,
            int internalFormat, int samples) {
        
        RenderBuffer depthBuffer = RenderBuffer.createDepthBuffer(gl, w, h,
                samples);
        RenderBuffer colorBuffer = RenderBuffer.createColorBuffer(gl, w, h,
     ...
     */
    public void attachColorTarget(GL gl, Texture2D texture, int level,
            int attachPoint, boolean autoDispose) {
        texWidth = texture.getWidth();
        texHeight = texture.getHeight();
        
        colorTextures[attachPoint] = texture;
        gl.glFramebufferTexture2D(GL.GL_FRAMEBUFFER, GL.GL_COLOR_ATTACHMENT0
                + attachPoint, GL.GL_TEXTURE_2D, texture.getID(), level);
    ...
    public void attachColorTarget(GL gl, RenderBuffer rbo, boolean autoDispose) {
        ...
        gl.glFramebufferRenderbuffer(GL.GL_FRAMEBUFFER,
                GL.GL_COLOR_ATTACHMENT0, GL.GL_RENDERBUFFER, rbo.getID());
    ...
    public void attachDepthTarget(GL gl, Texture2D texture, boolean autoDispose) {
        ...
        gl.glFramebufferTexture2D(GL.GL_FRAMEBUFFER, GL.GL_DEPTH_ATTACHMENT,
                GL.GL_TEXTURE_2D, texture.getID(), 0);
