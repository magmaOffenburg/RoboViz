...
public class ObjExporter {
   ...
   public static class Data {
       ...
       */
      public Data(float[][] vertices, float[][] normals, float[][] texCoords,
            int[][] faces, FaceType faceType, boolean zeroIndexed) {
   ...
   private class DataWriteThread extends Thread {
      ...
      private void writeToFile(BufferedWriter out) throws IOException {
         ...
         out
               .write(String.format("# Num. Vertices: %d\n",
                     data.vertices.length));
         out.write(String.format("# Num. Faces (%s): %d\n", data.faceType
               .toString(), data.faces.length));
         ...
         for (int i = 0; i < data.faces.length; i++)
            writeFace(data.faces[i], useTexCoords, useNormals, data.faceType,
                  out);
   ...
   private static void writeVertex(float[] v, BufferedWriter out)
         throws IOException {
      out.write(String.format("v %f %f %f\n", v[0], v[1], v[2]));
   }

   private static void writeNormal(float[] n, BufferedWriter out)
         throws IOException {
      out.write(String.format("vn %f %f %f\n", n[0], n[1], n[2]));
   }

   private static void writeTexCoord(float[] tc, BufferedWriter out)
         throws IOException {
         ...
         if (normals) {
            ...
            if (faceType == Data.FaceType.TriangleList) {
               out.write(String.format("f %d/%d/%d %d/%d/%d %d/%d/%d\n", f[0],
                     f[0], f[0], f[1], f[1], f[1], f[2], f[2], f[2]));
            }
         } else {
            // f v/vt v/vt v/vt
            if (faceType == Data.FaceType.TriangleList) {
