...
public class Gaussian {
   ...
   private static float gaussian(int n, float blurriness) {
      return (float) ((1.0f / Math.sqrt(2 * Math.PI * blurriness)) * Math
            .exp(-(n * n) / (2 * blurriness * blurriness)));
   }

   /** Calculates horizontal and vertical blur parameters */
   public static BlurParams[] calcBlurParams(float blurriness, int samples,
         int w, int h) {
   ...
   private static BlurParams calcBlurParams(float x, float y, int samples,
         float blurriness) {
