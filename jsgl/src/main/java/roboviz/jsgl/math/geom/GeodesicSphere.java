    ...
    private class Builder {
        ...
        private void addEdgeVerts() {
            ...
            for (int edge = 0; edge < icoEdgeVerts.length; edge++) {
                ...
                float[] step = { (b[0] - a[0]) / len, (b[1] - a[1]) / len,
                        (b[2] - a[2]) / len };
        ...
        private void addInnerVerts() {
            ...
            for (int tri = 0; tri < icoTriVerts.length; tri++) {
                ...
                float[] down = { (b[0] - a[0]) / len, (b[1] - a[1]) / len,
                        (b[2] - a[2]) / len };
                float[] right = { (c[0] - b[0]) / len, (c[1] - b[1]) / len,
                        (c[2] - b[2]) / len };
        ...
        private void calcEdgePointers(int icoTriIndex) {
            ...
            lp = flipL ? (curTriEdges[0] + 1) * level + 11 : curTriEdges[0]
                    * level + 12;
            bp = flipB ? (curTriEdges[1] + 1) * level + 11 : curTriEdges[1]
                    * level + 12;
            rp = flipR ? (curTriEdges[2] + 1) * level + 11 : curTriEdges[2]
                    * level + 12;
        ...
        private void calcTriIndices(int[] aux) {
            ...
            for (int row = 0; row < (level + 1); row++) {
                ...
                for (int tri = 0; tri < trisOnRow; tri++) {
                    int offset = tri / 2;
                    if (tri % 2 == 0) {
                        triangles[triIndex++] = new int[] {
                                aux[a[0] + offset], aux[a[1] + offset],
                                aux[a[2] + offset] };
                    } else {
                        triangles[triIndex++] = new int[] {
                                aux[b[0] + offset], aux[b[1] + offset],
                                aux[b[2] + offset] };
                    }
                }
